...
class TestGitImpl : GitImpl() {
  ...
  override fun createEditor(project: Project, root: VirtualFile, handler: GitLineHandler,
    ...
    val editor = object : GitInteractiveRebaseEditorHandler(project, root) {
      fun handleUnstructuredEditor(file: File): Boolean {
        val plainTextEditor = interactiveRebaseEditor!!.plainTextEditor
        return if (plainTextEditor != null) handleEditor(file, plainTextEditor) else super.handleUnstructuredEditor(file.path)
      }

      fun handleInteractiveEditor(file: File): Boolean {
        val entriesEditor = interactiveRebaseEditor!!.entriesEditor
        return if (entriesEditor != null) handleEditor(file, entriesEditor) else super.handleInteractiveEditor(file.path)
      }
